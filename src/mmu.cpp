/**
 * MemoryChunk Struct:
 *   next -> MemoryChunk* (ptr)
 *   prev -> MemoryChunk* (ptr0
 *   allocated -> Boolean (whether chunk is allocated or free)
 *   size -> size_t (int) size of chunk
 */
...
void* MemoryManager::malloc(size_t size) {
  ...
  for (MemoryChunk* chunk = first; chunk != 0 && result == 0; chunk = chunk->next)
    if (chunk->size > size && !chunk->allocated)
      result = chunk; // Sloth love chunk <3
