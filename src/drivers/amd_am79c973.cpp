  ...
  busControlRegisterDataPort(dev->portBase + 0x16)
{
  ...
  uint64_t MAC0 = MACAddressPort.Read() % 256;
  uint64_t MAC1 = MACAddressPort.Read() / 256;
...
uint32_t amd_am79c973::HandleInterrupt(uint32_t esp) {
  ...
  if ((temp & 0x2000) == 0x2000) printf("AMD am79c973 COLLISSION ERROR \n");
  ...
  if ((temp & 0x0100) == 0x0100) printf("AMD am79c973 IS DONE \n");
...
void amd_am79c973::Recieve() {
  ...
  for (; (recvBufferDescr[currentRecvBuffer].flags & 0x80000000) == 0; currentRecvBuffer = (currentRecvBuffer + 1) % 8) {
    if (!(recvBufferDescr[currentRecvBuffer].flags & 0x40000000)
      ...
      if (size > 64) // Remove Eth2 Checksum
        size -= 4;
